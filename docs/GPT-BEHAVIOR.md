...
- If a placeholder has a non-empty `options[]`, the GPT **must** present those values as a numbered list and wait for the landlord to pick one by number — never ask for free text input.
