...
|---|---|---|
| 1 | `#206` — properties.current_tenant_id FK — migration + write on tenant add/archive [G10] | Independent Supabase migration. Do it early so #208 is unblocked when you reach it. |
| 2 | `#210` — lazy loading — replace per-step loadContext with targeted deps; shrink /context to menu essentials [G6] | Architectural change that all subsequent flows must conform to. |
| 3 | `#201` — loadPlaceholderUnion internal dep — resolve placeholder union for (property_type, use_case) [G7] | Required by the generate_document rewrite. |
| 4 | `#202` — scope /documents/generate required-check to template union + honor default column [G9] | Small backend fix; feeds generate_document correctness. |
| 5 | `#207` — cross-flow chaining mechanism — add_tenant → generate → send [G3] | Mechanism used by #204, #208, #209. Build it before those flows. |
| 6 | `#205` — reply-to-edit at confirm table — clear field + dependents, re-ask, re-derive [G2] | Confirm table UX. Implement before #203 which builds the confirm table itself. |
| 7 | `#203` — rewrite generate_document flow — Flow 3b [G8] | The big one. Depends on #201, #210, #202, #205, and the already-merged dynamic steps engine (#199). |
| 8 | `#204` — generate_document Flow 3a — chained from add_tenant, implicit use_case=initial [G8 part 2] | Depends on #203 and #207. |
| 9 | `#208` — add_tenant — chain to generate + warn on active-tenant replace [G18] | Depends on #206 (migration) and #207 (chaining). |
| 10 | `#209` — add_property — new building inline (Flow 9) [G17] | Depends on #207 for the chain pattern. |
| 11 | `#200` — template_sync flow — interactive per-change config, Option B derived menus [G12] | Uses the already-merged dynamic steps engine (#199). Comes last so all flow engine patterns are established. |
...
6. Open a PR using the repo's PR template:
   - Read `.github/pull_request_template.md`
   - Fill every section (Summary, Related Issue, Changes Made, How to Test, Agent Review checklist)
   - Write the filled content to `/tmp/pr-body.md`
   - Then create the PR:
     ```bash
