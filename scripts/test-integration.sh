#!/usr/bin/env bash
# Tests that use the real local Supabase service boundary with external APIs mocked.
set -euo pipefail
...
if grep -Eq "ok \| 0 passed \| 0 failed" <<<"$OUTPUT"; then
  echo "ERROR: integration test filter matched zero tests. Check test names or filter ${FILTER}." >&2
  exit 1
fi

echo "==> test-integration.sh passed"
