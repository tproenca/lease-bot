...
Deno.serve(
  ...
  (req: Request) => {
    ...
    if (code) {
      resolveCode(code);
      return new Response("Autenticado! Pode fechar esta aba.", {
        status: 200,
      });
    }
...
if (!tokenJson.access_token) {
  console.error("Auth error:", tokenJson.error);
  Deno.exit(1);
}
...
async function findTemplatesFolder(): Promise<string | null> {
  ...
  url.searchParams.set(
    "q",
    `name = 'Templates' and mimeType = 'application/vnd.google-apps.folder' and trashed = false`,
  );
...
if (!docId) {
  ...
  if (!templatesFolderId) {
    console.warn("⚠ Templates folder not found — doc will be in Drive root.");
  }

  const r = await fetch("https://docs.googleapis.com/v1/documents", {
    method: "POST",
    ...driveAuth,
    body: JSON.stringify({ title: "Guia de Placeholders" }),
  });
  ...
  if (templatesFolderId) {
    ...
    console.log(
      `Created doc in Templates: https://docs.google.com/document/d/${docId}/edit`,
    );
  } else {
    console.log(
      `Created doc: https://docs.google.com/document/d/${docId}/edit`,
    );
  }
} else {
