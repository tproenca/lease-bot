...
BEGIN
  IF current_setting('app.service_role_key', true) IS NULL
    ...
    EXCEPTION WHEN insufficient_privilege THEN
      -- ALTER DATABASE SET requires superuser. In CI and managed Supabase
      -- the migration runner lacks this right — skip silently. Production
      -- values must be set manually via the Supabase dashboard.
      NULL;
    END;
  END IF;
END;
